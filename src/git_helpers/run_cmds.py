...
def set_global_config() -> None:
    ...
    shell_utils.run_cmd("git", "config", "--global", "pull.rebase", "false")
    ...
    shell_utils.run_cmd("git", "config", "--global", "pull.ff", "true")
    shell_utils.run_cmd("git", "config", "--global", "merge.ff", "true")
    ...
    shell_utils.run_cmd("git", "config", "--global", "rerere.enabled", "true")
    print("result: installed FF-first merge defaults globally in ~/.gitconfig")


def show_global_config() -> None:
...
def cmd_show_upstream() -> None:
    ...
    current_branch_name = inspect_repo.current_branch()
    ...
    upstream_name = shell_utils.query_cmd(
        ...
        "--symbolic-full-name",
        "@{u}",
    ) if inspect_repo.has_upstream() else ""
    if upstream_name:
        ...
        shell_utils.run_cmd("git", "log", "--oneline", "--decorate", "-1", upstream_name)
...
def cmd_branches_status() -> None:
    ...
    shell_utils.run_cmd("git", "fetch", "--prune", "--quiet")
    ...
    shell_utils.run_cmd("git", "branch", "-vv", "--no-abbrev")


def cmd_ahead_behind() -> None:
    ...
    if not inspect_repo.has_upstream():
        shell_utils.kill(f"no upstream set for {inspect_repo.current_branch()}")
    ...
    shell_utils.run_cmd("git", "fetch", "--quiet")
    ...
    if not ahead_behind_counts and shell_utils.dryrun:
        return
...
def cmd_unpulled_commits() -> None:
    ...
    shell_utils.run_cmd("git", "log", "--oneline", f"HEAD..{upstream_name}")


def cmd_local_remotes() -> None:
...
def cmd_show_recent_commits(max_entries: int = 20) -> None:
    ...
    shell_utils.run_cmd("git", "log", "--oneline", "--decorate", "-n", str(max_entries))


def cmd_submodules_status() -> None:
...
def cmd_rename_last_commit(
    ...
    shell_utils.run_cmd("git", "commit", "--amend", "-m", new_message)
...
def cmd_delete_local_branch(
    ...
    if inspect_repo.current_branch() == branch_name:
    ...
    shell_utils.run_cmd("git", "branch", "-d", "--", branch_name)
    shell_utils.log_outcome(f"deleted local branch '{branch_name}'")


def cmd_prune_gone_locals() -> None:
...
def cmd_prune_merged_locals(
    ...
    if not base_name:
        ...
        remote_name = inspect_repo.get_default_remote_name()
        default_branch_name = inspect_repo.get_default_branch_name()
...
def cmd_cleanup_local_branches(
    ...
    cmd_prune_gone_locals()
    cmd_prune_merged_locals(base_name)
...
def cmd_track_remote_branch(
    ...
    shell_utils.run_cmd("git", "fetch", "--prune", remote_branch.split("/")[0])
    ...
    shell_utils.run_cmd("git", "switch", "-c", local_branch_name, "--track", remote_branch)
...
def cmd_create_branch_from_default(
    ...
    shell_utils.run_cmd("git", "fetch", "--prune", remote_name)
    ...
    shell_utils.run_cmd("git", "push", "-u", remote_name, "HEAD")
...
def cmd_create_branch_from_remote(
    ...
    shell_utils.run_cmd("git", "switch", "-c", new_branch_name, "--no-track", start_ref)
    shell_utils.log_step("publishing branch and setting upstream (-u)")
...
def cmd_push(
    ...
    if inspect_repo.has_upstream():
        ...
        shell_utils.run_cmd("git", "push", *extra_args)
    else:
...
def cmd_sync_branch(
    ...
    shell_utils.log_step("verifying clean worktree (unless GIT_ALLOW_DIRTY=1)")
    inspect_repo.ensure_clean_worktree()
    ...
    if base_name:
        ...
        shell_utils.run_cmd("git", "merge", "--ff", base_name)
